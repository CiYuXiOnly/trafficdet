...
feature_names=['fiat_mean','fiat_min','fiat_max','fiat_std','biat_mean','biat_min','biat_max','biat_std',
...
def get_flow_feature_from_pcap(pcapname, writer):
    ...
    pid = os.getpid()
    print(">>> {} has {} flows pid={}".format(pcapname,len(flows.values()),pid))
    ...
    for flow in flows.values():
        ...
        if feature == None:
            ...
            continue
        if feature != None and (all(v is None for v in feature)):
            print("invalid flow {}:{}->{}:{}".format(flow.src,flow.sport,flow.dst,flow.dport))
            continue
        
        try:  
            feature = [flow.src,flow.sport,flow.dst,flow.dport] + feature
            # feature = [cnt] + [flow.src,flow.sport,flow.dst,flow.dport] + feature
            for item in feature:
                if item == None:
                    item = 'NAN'
            writer.writerow(feature)  
        except Exception as e:  
            print("Failed to write flow:", e)  
        cnt += 1
        # print(feature[6:10])    
        # print('========i:{}========'.format(cnt))  
    
    print('>>> Totally {} flows written!'.format(cnt))
